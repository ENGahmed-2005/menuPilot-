...
import { getSessionOrdersNormalized } from "../api/orders";
...
export function useOrderTracking(sessionId) {
  ...
  useEffect(() => {
    ...
    getSessionOrdersNormalized(sessionId).then((data) => { if (active) { setOrders(data ?? []); setError(null); setLoading(false); } }).catch((err) => { if (active) { setError(err); setLoading(false); } });
