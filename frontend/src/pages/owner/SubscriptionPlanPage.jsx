...
/** نافذة تأكيد قبل تبديل الباقة — محاكاة خطوة الدفع/الفوترة اللي هتيجي من
 *  بوابة دفع حقيقية (Stripe مثلًا) لاحقًا. من غير الخطوة دي، أي مستخدم كان
 *  يقدر يترقّى لباقة أعلى بضغطة واحدة بدون أي احتكاك أو دفع فعلي، وده مش
 *  سلوك واقعي حتى في بيئة تجريبية. */
function ConfirmPlanModal({ plan, isUpgrade, onConfirm, onCancel, loading }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/60 p-4 backdrop-blur-sm" onClick={onCancel}>
      <div dir="rtl" className="relative w-full max-w-sm rounded-3xl bg-paper p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="mt-6 flex gap-3">
          <button type="button" onClick={onCancel} className="flex-1 rounded-full border border-ink/12 px-4 py-3 text-sm font-black text-ink-soft transition hover:bg-ink/5">تراجع</button>
          <button
            type="button"
            onClick={onConfirm}
            disabled={loading}
            className="flex flex-1 items-center justify-center gap-2 rounded-full bg-copper px-4 py-3 text-sm font-black text-ink transition hover:bg-copper-deep disabled:cursor-wait disabled:opacity-60"
          >
        ...
        <div className="flex flex-col items-end gap-3">
          ...
          {isCurrentPlan ? (
            ...
            <button
              ...
              className="flex items-center gap-2 rounded-full bg-copper px-5 py-3 text-sm font-black text-ink transition hover:bg-copper-deep"
            >
