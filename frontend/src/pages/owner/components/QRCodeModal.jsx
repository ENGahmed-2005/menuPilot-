    ...
    <div className="fixed inset-0 z-50 grid place-items-center bg-ink/65 p-4 backdrop-blur-sm" onMouseDown={onClose}>
      <div dir="rtl" className="w-full max-w-md rounded-[2rem] bg-paper p-6 shadow-2xl" onMouseDown={(e) => e.stopPropagation()}>
        <div className="flex items-start justify-between gap-4">
          <div><span className="inline-flex items-center gap-2 rounded-full bg-copper/10 px-3 py-1 text-xs font-black text-copper"><QrCode size={14}/> رمز QR</span><h2 className="mt-3 text-2xl font-black">{table.label}</h2><p className="mt-1 text-xs text-ink-soft/55">يستخدم لفتح منيو الطاولة من داخل المطعم.</p></div>
