...
import { Check, Search, ShoppingBag, Plus, Utensils, ChevronLeft } from "lucide-react";
...
export default function Menu() {
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-paper-2 pb-32 text-ink">
      <header className="relative overflow-hidden bg-ink text-paper">
        ...
        <div className="mx-auto max-w-5xl px-5 pb-8 pt-6 sm:px-8 sm:pt-8">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => navigate(cartPath)} aria-label="فتح السلة" className="relative grid h-11 w-11 place-items-center rounded-2xl border border-paper/10 bg-paper/5 transition hover:bg-paper/10">
              <ShoppingBag size={20} />
      ...
      {addedItem && (
        <div role="status" className="fixed inset-x-4 top-4 z-50 mx-auto flex max-w-sm items-center justify-center gap-2 rounded-2xl bg-herb px-4 py-3 text-sm font-bold text-paper shadow-xl">
          <Check size={17} /> تمت إضافة الطبق إلى السلة
        </div>
      )}
      ...
      <main className="mx-auto max-w-5xl px-5 sm:px-8">
        <div className="-mt-1 flex gap-2 overflow-x-auto py-5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {categories.map((category) => (
            <button key={category} onClick={() => setActiveCategory(category)} aria-pressed={activeCategory === category} className={`whitespace-nowrap rounded-full px-5 py-2.5 text-sm font-bold transition ${activeCategory === category ? "bg-ink text-paper shadow-md" : "border border-ink/10 bg-paper text-ink-soft hover:border-ink/20"}`}>
              {category}
            </button>
          ))}
        </div>

        <div className="mb-5 flex items-end justify-between"><div><p className="text-xs font-medium text-ink-soft/60">اكتشف قائمتنا</p><h2 className="mt-1 text-2xl">الأطباق المتاحة</h2></div><span className="text-xs text-ink-soft/55">{filteredItems.length} طبق</span></div>
      ...
      {cartCount > 0 && (
        <div className="fixed inset-x-4 bottom-4 z-40 mx-auto max-w-5xl">
          <button onClick={() => navigate(cartPath)} className="flex w-full items-center justify-between rounded-2xl bg-ink px-5 py-4 text-paper shadow-2xl shadow-black/20 transition hover:-translate-y-0.5">
