import { Minus, Plus, Trash2, ShoppingBag, ArrowRight, Send, AlertCircle } from "lucide-react";
...
export default function Cart() {
  ...
  async function handleSubmitOrder() {
    setError("");
    if (!sessionId) {
      setError("جلسة الطعام غير موجودة. ارجع إلى القائمة وابدأ جلسة جديدة.");
      return;
    }
    ...
    try {
      ...
      setError(err.message || "تعذّر إرسال طلبك. حاول مرة أخرى.");
    } finally {
      setLoading(false);
    }
  }
    ...
    return (
      <div dir="rtl" className="grid min-h-screen place-items-center bg-paper-2 px-6 text-center text-ink">
        <div className="max-w-sm">
          ...
          <button onClick={() => navigate(menuPath)} className="mt-6 inline-flex items-center gap-2 rounded-2xl bg-ink px-5 py-3 text-sm font-bold text-paper"><ArrowRight size={17} /> العودة للقائمة</button>
        </div>
      </div>
    );
  }
    ...
    <div dir="rtl" className="min-h-screen bg-paper-2 pb-36 text-ink">
      <header className="bg-ink text-paper">
        <div className="mx-auto flex max-w-3xl items-center gap-4 px-5 py-6 sm:px-8">
          <button onClick={() => navigate(menuPath)} aria-label="العودة للقائمة" className="grid h-10 w-10 place-items-center rounded-xl border border-paper/10 bg-paper/5 transition hover:bg-paper/10"><ArrowRight size={19} /></button>
      ...
      <main className="mx-auto max-w-3xl px-5 py-6 sm:px-8">
        {error && (
          <p role="alert" className="mb-5 flex items-start gap-2 rounded-2xl border border-brick/15 bg-brick/10 px-4 py-3 text-sm leading-6 text-brick">
        ...
        <ul className="space-y-3">
          {items.map((it, i) => (
            <li key={it.menuItemId ?? i} className="rounded-3xl border border-ink/10 bg-paper p-4 shadow-sm">
              <div className="flex gap-4">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-start justify-between gap-3"><h3 className="truncate text-lg">{it.name}</h3><button onClick={() => removeItem(i)} aria-label={`إزالة ${it.name}`} className="text-brick/70 transition hover:text-brick"><Trash2 size={18} /></button></div>
                  <p className="mt-1 text-sm font-bold text-copper-deep">{(Number(it.price) * Number(it.quantity)).toFixed(2)} ₪</p>
                  <div className="mt-3 flex items-center justify-between gap-3">
                    <div className="flex items-center gap-2 rounded-xl bg-ink px-2 py-1.5 text-paper"><button aria-label={`تقليل كمية ${it.name}`} onClick={() => updateQuantity(i, Math.max(1, it.quantity - 1))} className="grid h-7 w-7 place-items-center rounded-lg hover:bg-paper/10"><Minus size={15} /></button><b className="min-w-5 text-center text-sm">{it.quantity}</b><button aria-label={`زيادة كمية ${it.name}`} onClick={() => updateQuantity(i, it.quantity + 1)} className="grid h-7 w-7 place-items-center rounded-lg hover:bg-paper/10"><Plus size={15} /></button></div>
                  </div>
                </div>
              </div>
              <input aria-label={`ملاحظة على ${it.name}`} placeholder="أضف ملاحظة للطبق (اختياري)" value={it.note || ""} onChange={(e) => updateNote(i, e.target.value)} className="mt-4 w-full rounded-2xl border border-ink/10 bg-paper-2 px-4 py-3 text-sm outline-none transition focus:border-copper focus:ring-2 focus:ring-copper/15" />
            </li>
          ))}
        </ul>
      ...
      <div className="fixed inset-x-0 bottom-0 z-40 border-t border-ink/10 bg-paper-2/95 p-4 backdrop-blur">
        <div className="mx-auto flex max-w-3xl items-center gap-3">
          ...
          <button onClick={handleSubmitOrder} disabled={loading || !sessionId} className="flex flex-1 items-center justify-center gap-2 rounded-2xl bg-copper py-3.5 text-sm font-black text-ink shadow-lg shadow-copper/10 transition hover:bg-copper-deep disabled:cursor-not-allowed disabled:opacity-50"><Send size={17} />{loading ? "جارِ إرسال الطلب…" : "تأكيد وإرسال الطلب"}</button>
        </div>
      </div>
    </div>
  );
