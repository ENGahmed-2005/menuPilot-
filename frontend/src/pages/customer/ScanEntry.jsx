...
export default function ScanEntry() {
  ...
  async function handleSubmit(e) {
    ...
    if (!trimmedName || !trimmedPhone) {
      setError("يرجى إدخال الاسم ورقم الهاتف للمتابعة.");
      return;
    }
    ...
    try {
      ...
      if (!session?.id) {
        throw new Error("تعذّر الحصول على رقم جلسة الطعام.");
      }
      ...
      <div className="mx-auto flex min-h-screen w-full max-w-6xl flex-col lg:flex-row">
        <section className="relative hidden overflow-hidden lg:flex lg:w-1/2 lg:flex-col lg:justify-between lg:p-12">
          ...
          <div className="relative">
            <div className="mb-10 flex h-12 w-12 items-center justify-center rounded-2xl bg-copper text-ink shadow-lg"><Utensils size={22} /></div>
            <p className="text-sm font-semibold tracking-[0.2em] text-copper">MENUPILOT</p>
            <h1 className="mt-5 max-w-lg font-display text-6xl leading-tight">طلبك يبدأ من هنا.</h1>
            <p className="mt-5 max-w-md text-base leading-8 text-paper/60">أدخل بياناتك مرة واحدة، ثم تصفح القائمة واطلب مباشرة من طاولتك بدون تطبيق.</p>
          </div>
          <div className="relative flex items-center gap-3 text-sm text-paper/50"><CheckCircle2 size={18} className="text-herb" />تجربة سريعة • بدون تسجيل حساب</div>
        </section>

        <section className="flex flex-1 items-center justify-center px-5 py-8 sm:px-8 lg:bg-paper-2 lg:text-ink">
          <div className="w-full max-w-md">
            <div className="mb-6 flex items-center gap-3 lg:hidden"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-copper text-ink"><Utensils size={19} /></div><div><p className="text-xs font-bold tracking-[0.18em] text-copper">MENUPILOT</p><p className="text-xs text-paper/50">القائمة الرقمية</p></div></div>
