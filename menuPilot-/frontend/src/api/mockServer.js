...
export async function mockRequest(method, rawPath, body, token) {
  ...
  if (method === "POST" && path === "/auth/register") {
    ...
    // owner جديد يبدأ على الباقة الأساسية (basic) — نمط SaaS معتاد: تجربة
    // مجانية/رخيصة الأول، والترقية اختيارية من صفحة الاشتراك.
    const account = {
      email: body.email, password: body.password, role: "owner",
      restaurantName: body.restaurant_name, plan: "basic", theme: null,
    };
    owners.push(account);
