...
export default function Sidebar() {
  ...
  const links = (LINKS_BY_ROLE[role] || []).filter((link) => hasPlanFeature(planId, link.feature));

  return <>
    <div className="flex items-center justify-between border-b border-paper/10 bg-ink px-4 py-3 text-paper lg:hidden"><div><span className="font-display text-xl">menuPilot</span><span className="mr-2 rounded-full bg-paper/10 px-2 py-0.5 text-[10px] font-bold text-copper">{plan.name}</span></div><button type="button" onClick={() => setOpen(true)} aria-label="فتح القائمة" className="rounded-lg p-2 hover:bg-paper/10">☰</button></div>
    ...
    <aside className={`fixed inset-y-0 right-0 z-50 flex w-72 flex-col bg-ink text-paper shadow-2xl transition-transform lg:sticky lg:top-0 lg:h-screen lg:translate-x-0 lg:shadow-none ${open ? "translate-x-0" : "translate-x-full"}`}>
      <div className="border-b border-paper/10 px-6 py-5"><div className="flex items-center justify-between"><span className="font-display text-2xl">menuPilot</span><button onClick={() => setOpen(false)} className="lg:hidden">×</button></div><div className="mt-4 rounded-2xl border border-copper/20 bg-copper/10 p-3"><div className="flex items-center justify-between"><span className="text-xs text-paper/50">الباقة الحالية</span><span className="rounded-full bg-copper px-2.5 py-1 text-[10px] font-black text-ink">{plan.name}</span></div><div className="mt-2 text-lg font-black">${plan.price}<span className="text-[10px] font-normal text-paper/45"> / شهريًا</span></div></div></div>
