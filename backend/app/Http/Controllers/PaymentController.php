...
class PaymentController extends Controller
{
    ...
    private function sessionWithRestaurant($sessionId)
    {
        return DB::table('dining_sessions')
            ...
            ->where('dining_sessions.id', $sessionId)
            ->select('dining_sessions.*', 'restaurant_tables.user_id as restaurant_user_id', 'users.payment_methods')
            ->first();
    }

    private function total($orderId)
    {
        return (float) DB::table('order_items')->where('order_id', $orderId)->sum(DB::raw('quantity * unit_price'));
    }
    ...
    public function options($sessionId)
    {
        ...
        return $this->out([
            ...
            'bank' => [
                'id' => 'bank', 'name' => $configured['bank']['name'] ?? 'تحويل بنكي',
                'enabled' => (bool) ($configured['bank']['enabled'] ?? false),
                'account_name' => $configured['bank']['account_name'] ?? null,
                'account_number' => $configured['bank']['account_number'] ?? null,
                'qr_url' => $configured['bank']['qr_url'] ?? null,
            ],
            'wallet' => [
                'id' => 'wallet', 'name' => $configured['wallet']['name'] ?? 'محفظة إلكترونية',
                'enabled' => (bool) ($configured['wallet']['enabled'] ?? false),
                'account_name' => $configured['wallet']['account_name'] ?? null,
                'account_number' => $configured['wallet']['account_number'] ?? null,
                'qr_url' => $configured['wallet']['qr_url'] ?? null,
            ],
        ]);
    }
    ...
    public function submit(Request $request, $sessionId)
    {
        $v = $request->validate([
            ...
            'payer_name' => 'required|string|min:2|max:255',
            'payer_phone' => 'required|string|min:7|max:50',
        ...
        $result = DB::transaction(function () use ($v, $sessionId, $session, $proofPath) {
            $orderId = DB::table('orders')->insertGetId([
                'dining_session_id' => $sessionId, 'user_id' => $session->restaurant_user_id,
            ...
            foreach ($v['items'] as $item) {
                ...
                DB::table('order_items')->insert([
                    'order_id' => $orderId, 'menu_item_id' => $menuItem->id, 'quantity' => $item['quantity'],
                    'unit_price' => $menuItem->price, 'note' => $item['note'] ?? null, 'status' => 'active',
            ...
            $paymentId = DB::table('payments')->insertGetId([
                'dining_session_id' => $sessionId, 'order_id' => $orderId, 'method' => $v['method'], 'status' => 'pending',
                'provider' => $v['provider'] ?? null, 'payer_name' => trim($v['payer_name']), 'payer_phone' => trim($v['payer_phone']),
                'proof_path' => $proofPath, 'amount' => $this->total($orderId), 'created_at' => now(), 'updated_at' => now(),
            ]);
    ...
    public function pending(Request $request)
    {
        $payments = DB::table('payments')
            ...
            ->select('payments.*', 'orders.status as order_status', 'restaurant_tables.label as table_label', 'dining_sessions.customer_name')
            ->latest('payments.id')
            ->get();
    ...
    public function verify(Request $request, $id)
    {
        $payment = DB::table('payments')
            ->join('dining_sessions', 'dining_sessions.id', '=', 'payments.dining_session_id')
            ->join('restaurant_tables', 'restaurant_tables.id', '=', 'dining_sessions.restaurant_table_id')
            ->where('payments.id', $id)->where('restaurant_tables.user_id', $request->user()->id)
