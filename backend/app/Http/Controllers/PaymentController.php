...
class PaymentController extends Controller
{
    ...
    public function options($sessionId)
    {
        ...
        return $this->out([
            ...
            'bank' => [
                'id' => 'bank',
                'name' => $configured['bank']['name'] ?? 'تحويل بنكي',
            ...
            'wallet' => [
                'id' => 'wallet',
                'name' => $configured['wallet']['name'] ?? 'محفظة إلكترونية',
    ...
    public function submit(Request $request, $sessionId)
    {
        ...
        $result = DB::transaction(function () use ($v, $sessionId, $session, $proofPath) {
            $orderId = DB::table('orders')->insertGetId([
                ...
                'user_id' => $session->restaurant_user_id,
            ...
            foreach ($v['items'] as $item) {
                $menuItem = DB::table('menu_items')
                    ...
                    ->where('user_id', $session->restaurant_user_id)
            ...
            $paymentId = DB::table('payments')->insertGetId([
                ...
                'payer_name' => trim($v['payer_name']),
                'payer_phone' => trim($v['payer_phone']),
                'proof_path' => $proofPath,
                'amount' => $this->total($orderId),
                'created_at' => now(),
                'updated_at' => now(),
            ]);
    ...
    public function verify(Request $request, $id)
    {
        ...
        DB::transaction(function () use ($payment, $request) {
            ...
            DB::table('dining_sessions')->where('id', $payment->dining_session_id)->update([
                'status' => 'ordering',
                'updated_at' => now(),
            ]);
        });
