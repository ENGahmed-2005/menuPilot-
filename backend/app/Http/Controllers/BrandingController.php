...
class BrandingController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        if (!$user->hasFeature('branding')) {
            return response()->json(['message' => 'Branding customization requires an active trial or paid plan.'], 403);
        }
        ...
        foreach (['logo' => 'logo_url', 'background' => 'background_url'] as $file => $column) {
            if ($request->hasFile($file)) {
                if ($settings->{$column}) {
                    ...
                    if ($old) Storage::disk('public')->delete(ltrim(str_replace('/storage/', '', $old), '/'));
                }
    ...
    public function reset(Request $request)
    {
        ...
        $settings->update([
            'primary_color' => '#B8793E', 'secondary_color' => '#5B7A52',
            'text_color' => '#171717', 'button_color' => '#171717',
            'card_style' => 'rounded', 'font_family' => 'system',
