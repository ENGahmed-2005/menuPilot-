...
class MenuController extends Controller{
 ...
 public function public($code){$t=DB::table('restaurant_tables')->where('table_code',$code)->first();if(!$t)return response()->json(['message'=>'Invalid table code'],404);return $this->out(['table'=>$t,'items'=>DB::table('menu_items')->where('user_id',$t->user_id)->where('is_available',true)->orderBy('category')->orderBy('name')->get()]);}
}
