...
class BillingController extends Controller
{
    ...
    public function bill($sid)
    {
        $items = DB::table('order_items')->join('orders', 'orders.id', '=', 'order_items.order_id')->join('menu_items', 'menu_items.id', '=', 'order_items.menu_item_id')->where('orders.dining_session_id', $sid)->where('order_items.status', 'active')->select('order_items.id', 'menu_items.name', 'order_items.quantity', 'order_items.unit_price', DB::raw('(order_items.quantity*order_items.unit_price) as total'))->get();
    ...
    public function pay(Request $r, $sid)
    {
        ...
        DB::table('restaurant_tables')->where('id', $s->restaurant_table_id)->update(['status' => 'available', 'updated_at' => now()]);
