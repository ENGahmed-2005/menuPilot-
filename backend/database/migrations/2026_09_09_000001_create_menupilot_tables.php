...
return new class extends Migration
{
    public function up(): void
    {
        Schema::table('users', function (Blueprint $t) {
            $t->string('restaurant_name')->nullable()->after('name');
            $t->string('plan')->default('starter')->after('restaurant_name');
            $t->string('role')->default('owner')->after('plan');
            $t->string('api_token', 80)->nullable()->unique()->after('password');
            $t->json('theme')->nullable()->after('api_token');
        });
        Schema::create('restaurant_tables', function (Blueprint $t) {
            $t->id();
            $t->foreignId('user_id')->constrained()->cascadeOnDelete();
            $t->string('label');
            $t->unsignedInteger('seats')->default(2);
            $t->string('table_code', 80)->unique();
        ...
        Schema::create('staff', function (Blueprint $t) {
            ...
            $t->string('role')->default('waiter');
            $t->timestamps();
        });
        Schema::create('dining_sessions', function (Blueprint $t) {
            $t->id();
            $t->foreignId('restaurant_table_id')->constrained('restaurant_tables')->cascadeOnDelete();
        ...
        Schema::create('orders', function (Blueprint $t) {
            $t->id();
            $t->foreignId('dining_session_id')->constrained('dining_sessions')->cascadeOnDelete();
            $t->foreignId('user_id')->constrained()->cascadeOnDelete();
        ...
        Schema::create('order_items', function (Blueprint $t) {
            ...
            $t->foreignId('reassigned_to_session_id')->nullable()->constrained('dining_sessions')->nullOnDelete();
            $t->timestamps();
        });
